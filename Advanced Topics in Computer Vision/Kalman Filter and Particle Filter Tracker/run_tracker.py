...
from particle_filter_tracker import PFTracker, PFTParams

# set the path to directory where you have the sequences
dataset_path = 'vot2014' # TODO: set to the dataet path on your disk
sequences = ['bolt', 'david', 'diving', 'drunk', 'polarbear', 'sphere'] 
...
for sequence in sequences:
    ...
    parameters = PFTParams('NCV', n_particles = 20, q = 100)
    ...
    print('Average Overlap:', sum(overlaps)/len(overlaps))
...
for p in particles:
    ...
    parameters = PFTParams('NCV', n_particles = p, q = 100)
    tracker = PFTracker(parameters)
...
for qi in q:
    for model in models:
        ...
        parameters = PFTParams(model, n_particles = 50, q = qi)
        tracker = PFTracker(parameters)
